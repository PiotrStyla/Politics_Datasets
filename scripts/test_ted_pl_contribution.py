...
def test_dataset_contract_and_samples():
    ...
    assert all(item["source"] == SOURCE and item["license"] == LICENSE for item in data)
    assert all(item["author"] and item["text"].strip() and item["token_count"] > 0 for item in data)
    assert all("\ufffd" not in item["text"] for item in data)
...
def test_ontology_is_addressed_and_falsifiable():
    ...
    mapping = {
        ...
        "src/build_ted_pl.py": ROOT / "src/build_ted_pl.py",
        "src/test_ted_pl_contribution.py": ROOT / "src/test_ted_pl_contribution.py",
    }
