...
## Remaining Consultation Batch v0.1

`data/rcl_remaining_consultations_v0_1` processes the 42 queue rows not included
in the accepted 40-row pilot. The source queue was generated by
`scripts/rcl_make_remaining_source_queue.py` from the 82-row review queue after
excluding the 40 pilot document URLs.

Download evidence:

- source rows: 42
- download failures: 0
- media types: 33 DOCX, 8 PDF, 1 XLSX
- unique source URLs: 42
- unique artifact digests: 41
- duplicate content digest: 1 duplicate SHA-256 across two queue rows
...
- extracted: 41
- empty text: 1
- quality hints: 27 good, 2 usable, 12 poor, 1 not_extractable
- PII hints: 19 yes, 23 uncertain

Draft review suggestions:

- document types: 35 organization_comment, 7 government_response
- source types: 28 unknown, 7 public_body, 6 ngo, 1 employer_organization
- recommendations: 41 conditional, 1 exclude

This batch is not reviewed evidence yet. It is a local/source-processing batch
with machine observations and draft suggestions. Raw documents and extracted
